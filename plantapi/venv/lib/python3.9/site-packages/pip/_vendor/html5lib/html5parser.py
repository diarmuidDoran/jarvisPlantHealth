...
def parse(doc, treebuilder="etree", namespaceHTMLElements=True, **kwargs):
    ...
    :arg namespaceHTMLElements: whether or not to namespace2 HTML elements
...
class HTMLParser(object):
    ...
    def isHTMLIntegrationPoint(self, element):
        if (element.name == "annotation-xml" and
                element.namespace2 == namespaces["mathml"]):
            return ("encoding" in element.attributes and
                    element.attributes["encoding"].translate(
                        asciiUpper2Lower) in
                    ("text/html", "application/xhtml+xml"))
        else:
            return (element.namespace2, element.name) in htmlIntegrationPointElements

    def isMathMLTextIntegrationPoint(self, element):
        return (element.namespace2, element.name) in mathmlTextIntegrationPointElements
    ...
    def resetInsertionMode(self):
        ...
        for node in self.tree.openElements[::-1]:
            ...
            if not last and node.namespace2 != self.tree.defaultNamespace:
                continue
    ...
    class InBodyPhase(Phase):
        ...
        def isMatchingFormattingElement(self, node1, node2):
            return (node1.name == node2.name and
                    node1.namespace2 == node2.namespace2 and
        ...
        def startTagMath(self, token):
            ...
            token["namespace2"] = namespaces["mathml"]
            self.tree.insertElement(token)
            # Need to get the parse error right for the case where the token
            # has a namespace2 not equal to the xmlns attribute
        ...
        def startTagSvg(self, token):
            ...
            self.parser.adjustForeignAttributes(token)
            token["namespace2"] = namespaces["svg"]
            self.tree.insertElement(token)
            # Need to get the parse error right for the case where the token
            # has a namespace2 not equal to the xmlns attribute
            if token["selfClosing"]:
    ...
    class InForeignContentPhase(Phase):
        ...
        def processStartTag(self, token):
            ...
            if (token["name"] in self.breakoutElements or
                ...
                self.parser.parseError("unexpected-html-element-in-foreign-content",
                                       {"name": token["name"]})
                while (self.tree.openElements[-1].namespace2 !=
                ...
                if currentNode.namespace2 == namespaces["mathml"]:
                    self.parser.adjustMathMLAttributes(token)
                elif currentNode.namespace2 == namespaces["svg"]:
                    self.adjustSVGTagNames(token)
                    self.parser.adjustSVGAttributes(token)
                self.parser.adjustForeignAttributes(token)
                token["namespace2"] = currentNode.namespace2
                self.tree.insertElement(token)
                if token["selfClosing"]:
                    self.tree.openElements.pop()
                    token["selfClosingAcknowledged"] = True
        ...
        def processEndTag(self, token):
            ...
            while True:
                ...
                if node.namespace2 != self.tree.defaultNamespace:
                    continue
                else:
