...
def water_required_check(moisture_sensor_pin:int, plant_id:int, sensor_id:int, plant_health_attribute_id:int):
    print ("Waaaaaaaaaaaater" + str(plant_health_attribute_id))
    
    rooms =  requests.get(f'http://127.0.0.1:5000/documented_api/rooms')
    rooms = rooms.json()
    plant_health_attribute_response = requests.get(f'http://127.0.0.1:5000/documented_api/plants/{plant_id}/plant_health_attributes')
    ...
    if(sensor_reading <= 35):
        ...
        pump_on()
        message= "Moisture in plant soil: "+ str(sensor_reading) + "\nYour plant has been watered."
    ...
    elif(moisture_sensor.checkMoistureSensor(moisture_sensor_pin) >= 35 
    and moisture_sensor.checkMoistureSensor(moisture_sensor_pin) <= 70): 
        ...
        message = (str(plant_name) + "'s soil is damp. No actions Required.  \nLocation: " + str(plant_room))
        
        requests.post(f'http://127.0.0.1:5000/documented_api/notifications', json={
            "notification_details": str(message),
            "time_stamp": str(time),
            "plant_health_attribute_id": int4(plant_health_attribute_id)
        })
        send_email("jarvis.plants@gmail.com", message)  
        
    else:
        message = "You plant " + str(plant_name) + " needs checked.  \nLocation: " + str(plant_room)
